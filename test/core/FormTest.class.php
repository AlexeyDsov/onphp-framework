<?php
	...
	final class FormTest extends TestCase
	{
		...
		{
			$form = Form::create()
				->add(Primitive::integer('int')->setMin(7)->setMax(17))
				->add(Primitive::form('capital')->ofProto(TestCityEntityProto::me())->required())
				->add(Primitive::formsList('cities')->ofProto(TestCityEntityProto::me()));
				;

			//import empty
			$form->import(array());
			$this->assertEquals(array('capital' => Form::MISSING), $form->getErrors());
			$this->assertEquals(array('capital' => Form::MISSING), $form->getInnerErrors());
			$form->checkRules();
			$this->assertEquals(array('capital' => Form::MISSING), $form->getErrors());
			$this->assertEquals(array('capital' => Form::MISSING), $form->getInnerErrors());

			$form->clean()->dropAllErrors();
			//check no errors after drop errors
			$this->assertEquals(array(), $form->getErrors());
			...
			$importScope = array(
				'int' => '6',
				'capital' => array('name' => 'Rostov', 'large' => true, 'capital' => true),
				'cities' => array(
					array('name' => 'Leningrad', 'large' => true),
					array('name' => 'Sochi', 'large' => false),
					array(),
				),
			);
			$form->import($importScope);
			$errors = array('int' => Form::WRONG, 'cities' => Form::WRONG);
			$this->assertEquals($errors, $form->getErrors());

			$innerErrors = array(
				'int' => Form::WRONG,
				'cities' => array(
					2 => array('name' => Form::MISSING)
				)
			);
			$this->assertEquals($innerErrors, $form->getInnerErrors());

			//checking rules
			$form->checkRules();
			$errors['capital'] = Form::WRONG;
			$this->assertEquals($errors, $form->getErrors());
		}
	}
?>