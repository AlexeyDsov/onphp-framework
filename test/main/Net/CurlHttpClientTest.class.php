<?php
	final class CurlHttpClientTest extends TestCase
	{
		...
		public function testGetWithAdditionalGet()
		{
			...
			$request = $this->spawnRequest(HttpMethod::get())->
				setGet($get)->
			...
			$this->assertEquals(
				$this->generateString($get, array(), array(), ''),
				$response->getBody()
			);
		}
		...
		public function testPostAndFilesWithMultiCurl()
		{
			...
			$request1 = $this->spawnRequest(HttpMethod::post())->
				setGet($get)->
				setPost($post1);
			...
			$this->assertEquals(
				$this->generateString($get, $post1, array(), UrlParamsUtils::toString($post1)),
		 ...
		 * @return HttpRequest
		 */
		private function spawnRequest(HttpMethod $method)
		{
			return HttpRequest::create()
				->setUrl(HttpUrl::create()->parse(ONPHP_CURL_TEST_URL))
				->setMethod($method);
		}
