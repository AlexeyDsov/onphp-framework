		...
		public function checkRules()
		{
			...
			foreach ($this->getPrimitiveList() as $primitiveName => $primitive) {
				...
				if ($primitive instanceof PrimitiveRule) {
					...
					$error = $this->getError($primitiveName);
					if ($value = $primitive->getValue()) {
						foreach ((is_array($value) ? $value : array($value)) as $form) {
							$form->checkRules();
			...
			foreach ($rules as $rule) {
				if ($rule->import(null, $this)) {
					$this->markGood($rule->getName());
				} else {
					$this->markWrong($rule->getName());
				}
			}
